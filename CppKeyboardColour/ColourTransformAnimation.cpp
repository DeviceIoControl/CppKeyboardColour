...
ColourTransformAnimation::ColourTransformAnimation()
{
	...
	auto const redToGreen = m_patternGenerator.GenerateColourTransform(DeviceMask::Keyboard, red, green, STEPS, FRAME_DURATION_MS);
	m_frames.AddFrames(redToGreen);

	auto const greenToBlue = m_patternGenerator.GenerateColourTransform(DeviceMask::Keyboard, green, blue, STEPS, FRAME_DURATION_MS);
	m_frames.AddFrames(greenToBlue);

	auto const blueToRed = m_patternGenerator.GenerateColourTransform(DeviceMask::Keyboard, blue, red, STEPS, FRAME_DURATION_MS);
